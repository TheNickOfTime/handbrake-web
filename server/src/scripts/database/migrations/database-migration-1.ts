...
export default function DatabaseMigration1(database: Database) {
	...
	const jobs_data_columns = (database.pragma('table_info(jobs_data)') as { name: string }[]).map(
		(info) => info.name
	);
	...
	if (job_ids_type[0].type == 'TEXT') {
		...
		const migrationTransaction = database.transaction(() => {
			...
			oldTables.forEach((table) => {
				...
				logger.info(`[database] [migration-1] Renamed table '${table}' to '${new_table}'.`);
			});
