...
import SectionOverlay from '~components/root/overlay';
...
export default function CreateJob({ onClose }: Params) {
	const { presets, defaultPresets, socket, config } = useContext(PrimaryContext)!;
	...
	return (
		<SectionOverlay id={styles['create-new-job']}>
										...
										.map((file, index) => {
											...
											return (
												<tr key={index}>
													...
													<td className='input-cell' heading={file.path}>
														{inputText}
													</td>
													<td
														className='output-cell'
														heading={outputFile.path}
													>
														{outputText}
													</td>
												</tr>
